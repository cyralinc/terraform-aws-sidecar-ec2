...
resource "aws_lb_listener" "cyral-sidecar-lb-ls" {
  ...
  protocol          = contains(var.sidecar_tls_ports, tonumber(each.value)) ? "TLS" : "TCP"
  certificate_arn   = contains(var.sidecar_tls_ports, tonumber(each.value)) ? var.load_balancer_certificate_arn : null
