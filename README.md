...
### Configuration without MongoDB support
```hcl
module "cyral_sidecar" {
    ...
    # though you have to open the desired ports using `sidecar_ports`. See the
    # next section for MongoDB configuration.
    repositories_supported = ["snowflake", "postgresql", "mysql"]
...
```
### Configuration with MongoDB support
```hcl
module "cyral_sidecar" {
    source  = "cyralinc/sidecar-ec2/aws"  
    version = "~> 3.0" # terraform module version
...
| <a name="input_secret_manager_type"></a> [secret\_manager\_type](#input\_secret\_manager\_type) | Define secret manager type for sidecar_client_id and sidecar_client_secret. | `string` | `"aws"` | no |
