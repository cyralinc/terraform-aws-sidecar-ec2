...
| <a name="input_hc_vault_integration_id"></a> [hc\_vault\_integration\_id](#input\_hc\_vault\_integration\_id) | (Deprecated - unused in sidecars `v4.10+`) HashiCorp Vault integration ID. For control planes `v4.16+`, use parameter `vault_integration_id` in the resource `cyral_sidecar` in our Terraform provider. [Learn more](https://registry.terraform.io/providers/cyralinc/cyral/latest/docs/resources/sidecar). This can also be configured using the Cyral control plane UI, as described in the [Cyral Docs](https://cyral.com/docs/integrations/secrets/hashicorp-vault). | `string` | `""` | no |
...
| <a name="input_launch_template_tags_resource_types"></a> [launch\_template\_tags\_resource\_types](#input\_launch\_template\_tags\_resource\_types) | The resource types to set custom tags for during launch | `set(string)` | <pre>[<br>  "instance",<br>  "volume",<br>  "network-interface"<br>]</pre> | no |
